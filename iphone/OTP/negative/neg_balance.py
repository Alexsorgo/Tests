...
def neg_balance(self, login, password):
    try:
        # f_id = first_id()
        lang = 0
        while lang < 3:
        ...
        if lang == 0:
            ...
            for i in sp:
                ...
                if not re.findall(r'\d+\.\d{2}\s\w{3}', w.get_attribute('name')):
                    if re.findall(r'\w{3}', w1.get_attribute('name'))[0] == 'UAH':
                    ...
                    if re.findall(r'\w{3}', w.get_attribute('name'))[0] == 'UAH':
            ...
            # conf.click()
            self.assertTrue(self.driver.find_element_by_id('The entered amount exceeds the available balance'))
            ...
            third_step = self.driver.find_element_by_id('navBarNext')
            third_step.click()
            cont = self.driver.find_element_by_id('Продолжить')
            cont.click()
            ok_btn = self.driver.find_element_by_id('navBarOk')
            ok_btn.click()
            sleep(3)
            ...
            # conf.click()
            self.assertTrue(self.driver.find_element_by_id('Введенная сумма первышает доступный остаток'))
            ...
            third_step = self.driver.find_element_by_id('navBarNext')
            third_step.click()
            cont = self.driver.find_element_by_id('Продовжити')
            cont.click()
            ok_btn = self.driver.find_element_by_id('navBarOk')
            ok_btn.click()
            sleep(3)
            ...
            # conf.click()
            self.assertTrue(self.driver.find_element_by_id('Введена сума перевищує доступний залишок'))
